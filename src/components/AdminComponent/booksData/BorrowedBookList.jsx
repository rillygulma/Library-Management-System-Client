...
const BorrowedBookList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('https://fubk-library-management-sytem-server.onrender.com/api/admin/allborrowers', {
          headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const accessToken = localStorage.getItem('token');
      const apiUrl = `https://fubk-library-management-sytem-server.onrender.com/api/deleteborrowedUsers/${id}`;

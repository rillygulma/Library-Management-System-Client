...
const BorrowedBookList = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get('https://library-management-system-server-f0vl.onrender.com/api/admin/allborrowers', {
          headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const accessToken = localStorage.getItem('token');
      const apiUrl = `https://library-management-system-server-f0vl.onrender.com/api/deleteborrowedUsers/${id}`;

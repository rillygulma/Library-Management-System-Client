...
const ManageBooks = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const url = 'https://library-management-system-server-f0vl.onrender.com/api/users/allbooks';
  ...
  const handleDelete = async (bookId) => {
    try {
      const accessToken = localStorage.getItem('token');
      await axios.delete(`https://library-management-system-server-f0vl.onrender.com/api/admin/deletebook/${bookId}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'Content-Type': 'application/json'
        }

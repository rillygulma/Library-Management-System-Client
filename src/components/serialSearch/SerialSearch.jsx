...
const SerialSearch = () => {
  ...
  const handleSubmitSearch = async (e) => {
    ...
    try {
      ...
      const response = await axios.get('https://library-management-system-server-f0vl.onrender.com/api/users/serialsearch', {
        params: formData,
        headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('https://library-management-system-server-f0vl.onrender.com/api/users/borrowers', dataToSave, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
        },
  ...
  const checkIfUserHasBorrowed = async (userId) => {
    try {
      const accessToken = localStorage.getItem('token');
      const response = await axios.get(`https://library-management-system-server-f0vl.onrender.com/api/users/borrowersHistory/${userId}`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
        },
